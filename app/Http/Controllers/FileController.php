...
class FileController extends Controller
{
    public function myFiles(Request $request, string $folder = null)
    {
        ...
        if ($favourites == 1){
            $query->Join('starred_files', 'starred_files_id', '=', 'files.id')
    ...
    public function createFolder(StoreFolderRequest $request)
    {
            ...
            $parent->append($file); // $parent->appendNode($file);
    }
    ...
    public function store(StoreFileRequest $request)
    {
        ...
        if (!empty($fileTree)) {
            ...
            foreach ($data['files'] as $file) {
                ...
                $this->saveFile($fileTree, $parent, $user);
            }
        }
    }
    ...
    public function saveFileTree($fileTree, $parent, $user)
    {
        foreach ($fileTree as $name => $file) {
            if (is_array($file)) {
                ...
                $this->saveFileTree($fileTree, $folder, $user);
            } else {
    ...
    private function getDownloadUrl(array $ids, $zipName)
    {
        if (count($ids) == 1) {
            ...
            $files = File::query()->whereIn('id', $ids[0])->get();
            $url = $this->createZip($files);
